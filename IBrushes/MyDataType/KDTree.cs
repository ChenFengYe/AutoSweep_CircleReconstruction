...
namespace SmartCanvas
{
	public class KDTree
	{
		...
		public static KDTree MakeFromPoints (params MyVector2[] points)
		{
		...
		static int FindSplitPoint (MyVector2[] points, int[] inds, int stIndex, int enIndex, int axis)
		{
		...
		//	of its plane.
		public static int FindPivotIndex (MyVector2[] points, int[] inds, int stIndex, int enIndex, int axis)
		{
			int splitPoint = FindSplitPoint(points, inds, stIndex, enIndex, axis);
			// int splitPoint = Random.Range(stIndex, enIndex);

			MyVector2 pivot = points[inds[splitPoint]];
			...
			while (currPt <= endPt)
			{
				MyVector2 curr = points[inds[currPt]];
		...
		public int FindNearest (MyVector2 pt)
		{
		...
		void Search (MyVector2 pt, ref float bestSqSoFar, ref int bestIndex)
		{
			float mySqDist = (float)(pivot - pt).Length();
		...
		float DistFromSplitPlane (MyVector2 pt, MyVector2 planePt, int axis)
		{
