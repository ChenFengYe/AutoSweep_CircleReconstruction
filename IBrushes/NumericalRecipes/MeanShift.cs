...
namespace i3_ImageManipulator
{
    public class VectorN : IEquatable<VectorN>
    {
        ...
        {
            this._data = (double[])data.Clone();
        }
        public VectorN(VectorN p)
        {
            this._data = (double[])p._data.Clone();
        }
        ...
        {
            double result = 0;
            foreach (double val in this._data)
                result += val * val;
        ...
        static public VectorN operator *(VectorN v, double s)
        {
        ...
        static public VectorN operator *(double s, VectorN v)
        {
            VectorN result = new VectorN(v.Dim);
            for (int i = 0; i < v.Dim; ++i)
                result[i] = v[i] * s;
    ...
    public class MeanShift
    {
        ...
        public double H { get; set; }   // bandwidth.
        ...
        {
            List<double> densList = new List<double>();
            ...
            double[] densities = densList.ToArray();
            ...
            for (int i = 0; i < unique.Count; ++i)
            {
                ...
                for (int j = 0; j < centers.Count; ++j)
                {
                    if (((unique[i] - centers[j]) / this.H).LengthSquare() < 1)
                    {
                        ...
                        centers[j] = sum / (double)modesets[j].Count;
